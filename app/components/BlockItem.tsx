...
export function BlockItem({
  ...
  function applyInlineFormat(kind: 'bold' | 'italic' | 'highlight') {
    const el = ref.current
    if (!el) return
    const sel = window.getSelection()
    ...
    try {
      ...
      const beforeStart = start - markers[0].length
      const afterEnd = end + markers[1].length
      const hasWrappingBefore =
        beforeStart >= 0 && fullText.slice(beforeStart, start) === markers[0]
      const hasWrappingAfter =
        afterEnd <= fullText.length && fullText.slice(end, afterEnd) === markers[1]

      if (hasWrappingBefore && hasWrappingAfter) {
        // Toggle off formatting
        newText =
          fullText.slice(0, beforeStart) +
          selected +
          fullText.slice(afterEnd)
      } else {
        // Apply formatting
        newText =
  ...
  return (
    ...
    >
      ...
      {selectionInfo && (
        ...
        >
          <div className="flex items-center gap-1 rounded-md border bg-popover px-1.5 py-1 text-xs shadow-lg">
            <button
            ...
            <button
              className="rounded px-1.5 py-0.5 italic hover:bg-accent"
              onMouseDown={(e) => {
                e.preventDefault()
