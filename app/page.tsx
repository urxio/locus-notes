...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#07070f] text-zinc-100 selection:bg-orange-500/30 font-sans overflow-x-hidden">
      {/* Background gradient mesh */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-8%] left-[-8%]  w-[45%] h-[45%] rounded-full bg-orange-600/14 blur-[140px]" />
        <div className="absolute top-[15%]  right-[-6%] w-[35%] h-[55%] rounded-full bg-orange-500/8  blur-[120px]" />
        <div className="absolute bottom-[-8%] left-[18%]  w-[55%] h-[40%] rounded-full bg-orange-900/12 blur-[140px]" />
        <div className="absolute top-[45%]  left-[35%]  w-[30%] h-[35%] rounded-full bg-amber-600/6   blur-[100px]" />
      ...
      >
        <div className="container mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <Link
            href="/app"
            className="bg-orange-600 hover:bg-orange-500 text-white px-5 py-2 rounded-full text-sm font-medium transition-all shadow-lg shadow-orange-600/20 flex items-center gap-2 group border border-orange-500/50"
          >
            <span>Open App</span>
      ...
      <main className="relative z-10 pt-32 pb-20">
        ...
        <section className="container mx-auto px-6 md:px-12 pt-20 pb-32 flex flex-col items-center text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 text-orange-400 text-xs font-medium mb-8 backdrop-blur-sm"
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Link
              href="/app"
              className="bg-orange-600 hover:bg-orange-500 text-white px-8 py-3.5 rounded-full text-base font-medium transition-all shadow-xl shadow-orange-600/20 flex items-center gap-2 group w-full sm:w-auto justify-center border border-orange-500/50"
            >
          ...
          {/* ── App Preview Mockup ─────────────────────────────────── */}
          <div
            className="mt-24 w-full max-w-5xl relative rounded-2xl overflow-hidden border border-white/[0.10] shadow-2xl shadow-orange-900/20 flex flex-col md:flex-row group"
            ...
            <div className="flex-1 p-6 md:p-10 border-b md:border-b-0 md:border-r border-white/[0.07] relative overflow-hidden">
              ...
              <div className="space-y-6 transform group-hover:translate-x-1 transition-transform duration-500 ease-out relative z-10 w-full max-w-md">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="px-3 py-1 bg-white/[0.04] text-zinc-400 rounded-full border border-white/[0.08] text-xs font-mono flex items-center gap-1.5"><Hash className="w-3 h-3" />knowledge-graph</div>
                </div>
      ...
      >
        <div className="container mx-auto px-6 md:px-12 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <Link href="/app" className="text-zinc-500 hover:text-orange-400 font-medium text-sm transition-colors">
            Enter Workspace →
          </Link>
        </div>
      </footer>
...
function GlassFeatureCard({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div
      className="relative group overflow-hidden rounded-2xl p-8 border border-white/[0.07] hover:border-orange-500/30 transition-all duration-300"
      style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}
    >
      {/* Subtle inner glow on hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-orange-500/0 to-orange-500/0 group-hover:from-orange-500/[0.04] group-hover:to-transparent transition-all duration-500 rounded-2xl pointer-events-none" />
      <div className="relative z-10">
        <div className="w-12 h-12 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-orange-500/15 transition-all duration-300">
          {icon}
        </div>
